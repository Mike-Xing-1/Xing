%---------------Folded Dipole antenna simulation----------------------------------
clear all
close all
clc
...
dipole_length=0.24;    % 1/2 Folded dipole length
...
for k = zc-floor(Nd/2):zc+floor(Nd/2)
    if k ~= zc  % Keep excitation source at cz = 1
        ...
        cz(xc+1-folding_distance, yc, k) = 0.0;  % Folded conductor
    end
end

cz(xc+1, yc, zc) = 1.0;  % Central conductor excitation point
cz(xc+1-folding_distance, yc, zc) = 1.0;  % Folded conductor excitation point
...
dz(xc+1, yc, zc) = source(n);           
dz(xc+1-folding_distance, yc, zc) = source(n); 

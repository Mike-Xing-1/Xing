%---------------Folded Dipole Loop antenna simulation----------------------------------
clear all
close all
clc
...
loop_length = 0.12;  % Side length of the loop antenna
loop_radi = 0.01;  % Radius of the loop antenna
cell_x = 0.5 / 68;  % Grid size
loop_nd = floor(loop_length / cell_x) + 1;  % Number of grid units per side of the loop

dipole_length = 0.48;  % Dipole length
radi = 0.01;  % Dipole radius
Nd = floor(dipole_length / cell_x) + 1;
shift_y = -1;  % Shift of the dipole along the Y-axis in grid units
...
curr_loop=zeros(35,2048); % Initializing the dipole current
...
for layer = 1:length(layers)
    ...
    % Apply to the current spherical shell region
    for i = 1:NX
        for j = 1:NY
            for k = 1:NZ
...
% Define the four vertices of the square loop (now placed in the XZ plane)
x1 = xc - floor(loop_nd / 2);  % Starting point in X direction
x2 = xc + floor(loop_nd / 2);  % End point in X direction
z1 = zc - floor(loop_nd / 2);  % Starting point in Z direction
z2 = zc + floor(loop_nd / 2);  % End point in Z direction

% Define the four edges of the loop (loop in the XZ plane)
for i = x1:x2
    cz(i, yc, z1) = 0.0;  % Bottom edge (XZ plane)
    cz(i, yc, z2) = 0.0;  % Top edge (XZ plane)
end
for k = z1:z2
    cz(x1, yc, k) = 0.0;  % Left edge (XZ plane)
    cz(x2, yc, k) = 0.0;  % Right edge (XZ plane)
end

% Feed points at the midpoint of each edge
x_mid = xc;   % Midpoint in X direction (left and right edges)
z_mid = zc;   % Midpoint in Z direction (top and bottom edges)

% Place feed points at the midpoint of each edge
cz(x_mid, yc, z1) = 1.0;  % Bottom edge midpoint (XZ plane)
cz(x_mid, yc, z2) = 1.0;  % Top edge midpoint (XZ plane)
cz(x1, yc, z_mid) = 1.0;  % Left edge midpoint (XZ plane)
cz(x2, yc, z_mid) = 1.0;  % Right edge midpoint (XZ plane)

folding_distance = 2;

% Define the conductors of the dipole antenna along the Z axis and set source points with cz = 1
for k = zc-floor(Nd/2):zc+floor(Nd/2)
    if k ~= zc  % Keep excitation source at cz = 1
        cz(xc+1, yc, k) = 0.0;         % Central conductor
        cz(xc+1-folding_distance, yc + shift_y, k) = 0.0;  % Folded conductor
    end
end

cz(xc+1, yc, zc) = 1.0;  % Central conductor excitation point
cz(xc+1-folding_distance, yc + shift_y, zc) = 1.0;  % Folded conductor excitation point
...
t0 = 20.0;  % Time delay
tw = 6.0;  % Pulse width
source(n) = exp(-0.5 * (((Ts - t0) / tw)^2));  % Gaussian pulse

% Apply the feed at the midpoint of each edge
x_mid = xc;   % Midpoint in X direction (left and right edges)
z_mid = zc;   % Midpoint in Z direction (top and bottom edges)

dz(x_mid, yc, z1) = source(n);
dz(x_mid, yc, z2) = source(n);
dz(x1, yc, z_mid) = source(n);
dz(x2, yc, z_mid) = source(n);

% Apply the feed for the folded dipole
% Central conductor excitation
dz(xc+1, yc + shift_y, zc) = source(n); 
% Folded conductor excitation
dz(xc+1-folding_distance, yc + shift_y, zc) = source(n);
